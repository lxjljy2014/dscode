...
<template>
  ...
  >
    ...
    <div v-else class="w-full flex gap-3">
      <div
        class="mt-0.5 size-7 shrink-0 flex items-center justify-center rounded-full border border-line text-muted"
      >
        <span class="i-lucide:sparkles text-3.5" />
      </div>
      <div class="min-w-0 flex-1 whitespace-pre-wrap pt-1 text-sm leading-relaxed text-fg">
        {{ message.content }}<span v-if="message.streaming" class="ds-streaming-cursor" />
      </div>
    </div>
  </div>
</template>
